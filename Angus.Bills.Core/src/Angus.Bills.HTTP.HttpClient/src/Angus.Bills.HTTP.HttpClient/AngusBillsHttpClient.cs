...
{
    internal class AngusBillsHttpClient : IHttpClient
    {
        ...
        public AngusBillsHttpClient(System.Net.Http.HttpClient client, HttpClientOptions options)
        {
            _client = client;
            _options = options;
        }
        ...
        public Task<HttpResult<T>> SendResultAsync<T>(HttpRequestMessage request)
        {
            return Policy.Handle<Exception>()
                ...
                {
                    var response = await _client.SendAsync(request);
                    if (!response.IsSuccessStatusCode) return new HttpResult<T>(default, response);
        ...
        {
            if (stream is null || stream.CanRead is false) return default;
