...
namespace Angus.Bills.Tracing.Jaeger.RabbitMQ.Plugins
{
    internal sealed class JaegerPlugin : RabbitMqPlugin
    {
        private readonly ITracer _tracer;

        public JaegerPlugin(ITracer tracer)
        {
            _tracer = tracer;
        }
        ...
        private IScope BuildScope(string messageName, string serializedSpanContext)
        {
            ...
            if (string.IsNullOrEmpty(serializedSpanContext)) return spanBuilder.StartActive(true);
