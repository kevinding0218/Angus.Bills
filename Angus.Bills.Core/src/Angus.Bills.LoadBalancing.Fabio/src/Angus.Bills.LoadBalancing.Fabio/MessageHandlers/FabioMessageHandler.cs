...
namespace Angus.Bills.LoadBalancing.Fabio.MessageHandlers
{
    internal sealed class FabioMessageHandler : DelegatingHandler
    {
        ...
        {
            if (!_options.Enabled) return base.SendAsync(request, cancellationToken);
        ...
        {
            return new Uri($"{_options.Url}/{_servicePath}{request.RequestUri.Host}{request.RequestUri.PathAndQuery}");
        }
    }
}