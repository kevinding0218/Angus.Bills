...
namespace Drex.WebApi.Security
{
    internal sealed class CertificateMiddleware : IMiddleware
    {
        ...
        {
            if (!_options.Enabled) return next(context);

            if (IsAllowedHost(context)) return next(context);
            ...
            if (!_validateAcl) return next(context);
            ...
            if (acl.Permissions is null || !acl.Permissions.Any()) return next(context);

            if (_certificatePermissionValidator.HasAccess(certificate, acl.Permissions, context)) return next(context);
        ...
        private bool Verify(X509Certificate2 certificate)
        {
            ...
            foreach (var chainElement in chain.ChainElements) chainElement.Certificate.Dispose();

            if (chainBuilt) return true;
